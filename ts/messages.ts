export const showMessage = (messageType: string, itemName?: string): void => {
	...
	const iconContainer = document.querySelector('.message__icon') as HTMLSpanElement;
	...
	switch (messageType) {
		...
		case 'failedFetch':
			...
			iconContainer.insertAdjacentHTML(
				'afterbegin',
				`<svg viewBox="0 0 24 24" class="message__icon">
		...
		case 'successfulPurchase':
			...
			iconContainer.innerHTML = `<svg viewBox="0 0 24 24" class="message__icon">
